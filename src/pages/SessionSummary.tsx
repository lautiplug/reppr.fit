...
export default function SessionSummary() {
  ...
  return (
    <div className="min-h-screen bg-dark flex flex-col px-5 pt-16 pb-10">
      {/* Header */}
      <div className="flex flex-col items-center gap-3 mb-10">
        ...
        <p className="text-[#8E8E93] text-base text-center">
          {session.workoutName}
        </p>
      </div>
      ...
      <div className="flex gap-3 mb-8">
        ...
        <StatCard
          ...
          label="Ejercicios"
          value={`${completed.length}/${session.exercises.length}`}
        />
        <StatCard
      ...
      <div className="flex flex-col gap-3 mb-10 flex-1">
        {session.exercises.map((ex, i) => (
          <div
            key={i}
            className={`rounded-2xl px-4 py-3 flex items-center justify-between ${ex.skipped ? "bg-[#2C2C2E]/50" : "bg-[#2C2C2E]"}`}
          >
            <div className="flex items-center gap-3">
              ...
              <div>
                <p
                  className={`font-semibold text-sm ${ex.skipped ? "text-[#636366]" : "text-white"}`}
                >
                  {ex.name_es ?? ex.name}
                </p>
                {ex.skipped ? (
