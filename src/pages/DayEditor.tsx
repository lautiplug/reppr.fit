...
type SelectedExercise = DayExercise & {
...
export const DayEditor = () => {
  ...
  const firstMuscle: MuscleGroup =
    daySchedule?.type === "training"
      ? ((daySchedule.exercises[0]?.muscle_group ?? "chest") as MuscleGroup)
      : "chest";
  const [selectedMuscle, setSelectedMuscle] =
    useState<MuscleGroup>(firstMuscle);
  const [search, setSearch] = useState("");
  const [selectedExercises, setSelectedExercises] = useState<
    SelectedExercise[]
  >(
    daySchedule?.type === "training"
      ? daySchedule.exercises.map((e) => ({
          ...e,
          muscle_group: (e.muscle_group ?? "chest") as MuscleGroup,
          gif_url: e.gif_url,
        }))
      : [],
  );
  ...
  const initialName =
    daySchedule?.type === "training" ? daySchedule.workoutName : "";
  const initialSuggested = suggestWorkoutName(
    selectedExercises.map((e) => e.muscle_group),
  );
  ...
  const [nameEdited, setNameEdited] = useState(
    !!initialName && initialName !== initialSuggested,
  );
  const displayName = nameEdited ? workoutName : suggestedName;

  const debouncedSearch = useDebounce(search, 300);
  const { exercises, loading, loadingMore, hasMore, loadMore } = useExercises(
    selectedMuscle,
    debouncedSearch,
  );
  ...
  const handleSave = () => {
    if (!day) return;
    const uniqueMuscleLabels = [
      ...new Set(
        selectedExercises.map(
          (e) => MUSCLE_LABELS[e.muscle_group as MuscleGroup],
        ),
      ),
    ];
    const updated: DaySchedule = {
      ...
      muscleGroups:
        uniqueMuscleLabels.length > 0 ? uniqueMuscleLabels : ["Entreno"],
    ...
    const newSchedule = { ...schedule, [day as WeekDay]: updated };
    saveRoutine.mutate({
    ...
    <div className="pb-28">
      ...
      <div className="px-5 pt-2 pb-4 flex items-center gap-3">
        <button
          ...
          className="w-9 h-9 flex items-center justify-center rounded-full bg-[#3A3A3C]"
        >
        ...
        {loading ? (
          <div className="flex flex-col gap-3">
            {[...Array(6)].map((_, i) => (
              <div
                key={i}
                className="h-20 rounded-2xl bg-[#3A3A3C] animate-pulse"
              />
            ))}
          </div>
        ) : filtered.length === 0 ? (
          <p className="text-center text-[#8E8E93] text-sm py-10">
          ...
          filtered.map((ex) => {
            ...
            return (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <p
                    className={`font-bold text-sm leading-tight font-display ${selected ? "text-white" : "text-white"}`}
                  >
                    {ex.name_es ?? ex.name}
                  </p>
